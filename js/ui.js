...
window.NayePankhUI = (() => {
  ...
  const animateCounters = () => {
    ...
    counters.forEach(counter => {
      ...
      // Only animate if value is 0 or un-rendered
      if (count === 0) {
        updateCount();
      } else {
        counter.innerText = target.toLocaleString('en-IN');
  ...
  const renderCampaigns = (campaigns, onDonateClick) => {
    ...
    campaigns.forEach(c => {
      ...
      // Event listener on campaign click
  ...
  const renderWeeksTasks = (programTasks, completedTaskIds, onCheckboxChange) => {
    ...
    Object.keys(programTasks).forEach((weekKey, index) => {
      ...
      header.addEventListener('click', (e) => {
        // Prevent click if checked on inputs
  ...
  const drawCertificate = (name, role, duration, certId, xpPoints) => {
    ...
    // Wings graphic representation programmatically
    ctx.save();
    ctx.translate(w / 2, 90);
    
    // Draw wings path in gradient
    ...
    // 6. Awardee Name Display (Elegant Bold Brush Style font)
    ...
    // Let's position: Left = Verification ID, Center = Official Seal, Right = Signature
    const baselineY = 600;

    // 8a. Left: Verification ID
    ctx.textAlign = 'left';
    ctx.font = '600 11px monospace';
    ctx.fillStyle = '#64748B';
    ctx.fillText(`VERIFICATION ID: ${certId}`, 100, baselineY - 15);
    ctx.font = '11px "Inter"';
    ctx.fillText('Verify at: nayepankh.com/verify', 100, baselineY + 5);

    // 8b. Right: Signature lines
    ctx.textAlign = 'center';
    
    // Draw simulated signature graphics (squiggly lines representing a signature)
    ctx.strokeStyle = '#1D4ED8'; // Dark ink blue
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ...
    // 8c. Center: Official Ribbon Seal (Programmatic Vector Draw)
    ctx.save();
    ctx.translate(w / 2, baselineY + 15);
    
    // Draw seal ribbons (red/orange ribbons hanging down)
    ...
    // Draw spiked points border of the seal
    ...
    // Center emblem - wing symbol inside stamp
