...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Selected Active Navigation View (default: dashboard)
  ...
  const switchView = (viewName) => {
    ...
    if (viewName === 'dashboard') {
      viewTitle.textContent = "Impact Dashboard";
      viewSubtitle.textContent = "Badalte Bharat Ki Nayi Tasveer";
      ...
      viewTitle.textContent = "Certificate Desk";
      viewSubtitle.textContent = "Download your verified completion credentials";
      
      // Auto-populate certificate desk name with intern profile name if edited
  ...
  themeToggleBtn.addEventListener('click', () => {
    ...
    // Redraw certificate canvas to match rendering colors if open
  ...
  const updateHeaderStats = () => {
    ...
    // Calculate totals or fetch from campaign changes
    const totalRaised = state.campaigns.reduce((acc, c) => acc + c.raised, 0);

    // Update Guest User profile sidebar card
  ...
  // Handle donation payment method selection clicks
  ...
  // Donate modal submit form logic
  donationForm.addEventListener('submit', (e) => {
    ...
    // 1. Update State campaigns values
    ...
    state.donors.unshift(newDonor); // push to top
    if (state.donors.length > 6) state.donors.pop();

    // 3. If donation is simulated as completed by current user, grant XP as "Fundraising Support"!
    // Check if current user is submitting
    let xpGranted = 0;
    if (donorName.toLowerCase().includes('you') || donorName === state.userStats.name) {
      xpGranted = Math.min(Math.round(donorAmount / 10), 150); // 10% of amount, max 150XP
      grantXP(xpGranted);
      
      // Auto checkbox the fundraising task (w3t2) if it was not done!
      if (!state.completedTaskIds.includes('w3t2')) {
        state.completedTaskIds.push('w3t2');
        xpGranted += 120; // Task XP
        grantXP(120);
  ...
  const updateTaxCalculation = () => {
    ...
    // NayePankh offers 50% deduction on income tax outlay under 80G
    const exemptOutlay = Math.round(amount * 0.5);
    
    // Tax saved depends on slab rate (assume standard 30% slab rate estimate for presentation)
  ...
  const handleTaskCheckboxChange = (taskId, isChecked) => {
    ...
    if (isChecked) {
      ...
      if (index > -1) {
        ...
        grantXP(-targetTask.xp); // subtract
  ...
  const grantXP = (amount) => {
    ...
    // Level calculation formula (100XP per level, lvl max 5)
    // 0-199 Lvl 1, 200-399 Lvl 2, 400-599 Lvl 3, 600-799 Lvl 4, 800+ Lvl 5
    ...
    chatMessages.innerHTML = `
      <div class="chat-msg bot">
        ...
        <div class="msg-bubble">
          Hi! I'm the NayePankh Volunteer Outreach Assistant. I can write Whatsapp messages, emails, or LinkedIn copy for your campaign! 
          <br><br>
          Select presets in the left panel and click <strong>Auto-Generate Message</strong>, or type questions about fundraising tips!
        </div>
      </div>
    `;
  };
  ...
  chatInputForm.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      ...
      // Check QA keywords
      let botResponse = "I'm not sure I understand. I am specialized in fundraising templates, NayePankh details, 80G tax exemptions, and certificate requirements. Try selecting a template preset on the left or asking about 'fundraising tips'.";
  ...
  // Preset Auto-Generate button click handler
  generatePresetBtn.addEventListener('click', () => {
    ...
    // Append a simulated chat entry with a beautiful code box to copy
    toggleTypingIndicator(true);
  ...
  const updateCertificateChecklist = () => {
    ...
    // Check 1: Name updated (not "Guest Champion" or empty)
  ...
  // Cert form values change listener
